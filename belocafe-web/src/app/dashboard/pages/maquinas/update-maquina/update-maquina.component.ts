...
@Component({
  selector: 'app-update-maquina',
  templateUrl: './update-maquina.component.html',
  styleUrls: ['./update-maquina.component.css'],
})
export class UpdateMaquinaComponent {
  ...
  ngOnInit(): void {
    ...
    this.initializeForm(this.maquinaId);
  }

  initializeForm(id: number): void {
    this.maquinaService.getById(id).subscribe({
      next: (maquina) => {
        this.maquinaFormGroup.patchValue(maquina);
      },
    });
  }
