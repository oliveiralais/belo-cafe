...
@Component({
  selector: 'app-update-adubacao',
  templateUrl: './update-adubacao.component.html',
  styleUrls: ['./update-adubacao.component.css']
})
export class UpdateAdubacaoComponent {
  adubacaoFormGroup!: FormGroup;
  atividade!: Atividade;
  adubacao!: Adubacao;
  adubacaoId!: number;
  user!: User;
  ...
  constructor(
    ...
    this.adubacaoId = this.route.snapshot.params['id'];
  }

  ngOnInit(): void {
    this.adubacaoFormGroup = this.formBuilder.group({
    ...
    this.getAdubacao(this.adubacaoId);
  ...
  get insumo() {
    return this.adubacaoFormGroup.get('insumo');
  }

  get qtdeInsumo() {
    return this.adubacaoFormGroup.get('qtdeInsumo');
  }

  get ocorrenciaAno() {
    return this.adubacaoFormGroup.get('ocorrenciaAno');
  }

  get despesasExtras() {
    return this.adubacaoFormGroup.get('despesasExtras');
  }

  get statusAtividade() {
    return this.adubacaoFormGroup.get('statusAtividade');
  }

  get local() {
    return this.adubacaoFormGroup.get('local');
  }

  get observacoes() {
    return this.adubacaoFormGroup.get('observacoes');
  }

  get dataInicio() {
    return this.adubacaoFormGroup.get('dataInicio');
  }

  get dataFim() {
    return this.adubacaoFormGroup.get('dataFim');
  }
  ...
  getAdubacao(id: number): any {
    this.adubacaoService.getById(id).subscribe({
      next: (adubacao) => {
        this.adubacao = adubacao;
        this.atividade = adubacao.atividade;
        this.ocorrenciaAno!.setValue(adubacao.ocorrenciaAno);

        this.atividadeService.getById(adubacao.atividade.id!).subscribe({
          next: (atividade) => {
  ...
  getInsumos(): any {
    this.insumoService.findByTipoInsumoPaginate(3, +localStorage.getItem('user_id')!, 0, 100).subscribe({
      next: (insumos) => {
  ...
  onSubmit(): void {
    if (this.adubacaoFormGroup.invalid) {
      this.adubacaoFormGroup.markAllAsTouched();
      return;
    } else {
      ...
      this.adubacao = this.createAdubacao();

      this.adubacaoService.update(this.adubacao).subscribe({
        next: () => {
          this.router.navigate(['/dashboard/atividades/adubacoes']);
        },
