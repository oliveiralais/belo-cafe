...
@Component({
  selector: 'app-new-adubacao',
  templateUrl: './new-adubacao.component.html',
  styleUrls: ['./new-adubacao.component.css']
})
export class NewAdubacaoComponent {
  ...
  ngOnInit(): void {
    ...
    this.adubacaoFormGroup.get('insumo')?.valueChanges.subscribe((insumo) => {
      const qtdeInsumoControl = this.adubacaoFormGroup.get('qtdeInsumo');
      if (insumo) {
  ...
  onSubmit(): void {
    if (this.adubacaoFormGroup.invalid) {
      ...
      this.adubacaoService.save(this.adubacao).subscribe({
        next: () => {
