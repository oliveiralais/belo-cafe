...
@Component({
  selector: 'app-update-colheita',
  templateUrl: './update-colheita.component.html',
  styleUrls: ['./update-colheita.component.css']
})
export class UpdateColheitaComponent {
  ...
  constructor(
    ...
    this.colheitaId = this.route.snapshot.params['id'];
  }
  ...
  ngOnInit(): void {
    ...
    this.getColheita(this.colheitaId);
  ...
  getColheita(id: number): any {
    this.colheitaService.getById(id).subscribe({
      next: (colheita) => {
        this.colheita = colheita;
        this.atividade = colheita.atividade;

        this.tipoColheita!.setValue(colheita.tipoColheita);
        this.massaColhida!.setValue(colheita.massaColhida);
        this.rendimento!.setValue(colheita.rendimento);

        this.atividadeService.getById(colheita.atividade.id!).subscribe({
          next: (atividade) => {
  ...
  onSubmit(): void {
    if (this.colheitaFormGroup.invalid) {
      ...
      this.colheitaService.update(this.colheita).subscribe({
        next: () => {
